"""
Macro override using class definitions as containers for Macros.

This builds a class with macro names and macro bodies.
These classes are part of the app for defaults and application overrides.
The config file overrides are built as a class that inherits from the application overrides.
The WEB file @t definitions are built as a class that inherits from either

THEN.

A ``ChainMap`` includes the various sources:
[@t, config, app overrides, app defaults].

FINALLY.

A template can be built from the chain map to provide a single, definitive set of macros to use.
"""
